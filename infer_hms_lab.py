"""
infer_hms_lab_end2end.py —— DINOv3 + Lab a* 红墨水识别单模型端到端推理

流程：
    1. 加载 HMSLabRedSeg（RGB DINOv3 + Lab a* 辅助分支）
    2. 自适应滑动窗口推理 + 前景并集融合
    3. 输出单通道类别索引 mask
...
可选参数：
    python infer_hms_lab_end2end.py --limit N
"""
...
from HMS.transforms import rgb_to_lab_a, rgb_to_tensor, a_to_tensor
...
INFER = {
    ...
    "test_image_dir": "D:/lhb/datasets/testsets/HMS/testsets/test_images",

    # ========== 单模型 ==========
    "checkpoint_dir":  "./checkpoints/HMS_lab_2",
    "checkpoint_name": "best",       # best / last / 完整 .pt/.pth 路径

    # 0=background, 1=hd_w, 2=hd_y, 3=hd_t, 4=red
    "num_classes": 5,

    # ========== 模型结构（必须与训练时一致） ==========
    # backbone/lab 参数从 HMSConfig 读取，必须与训练时一致
    ...
    # ========== 输出 ==========
    # "output_dir": "D:/lhb/datasets/testsets/HMS/test/results_lab",  # 保存叠加结果的文件夹
    "output_dir": "D:/lhb/datasets/testsets/HMS/testsets/result/V2",  # 保存叠加结果的文件夹
...
CLASS_NAMES = {1: "hd_w", 2: "hd_y", 3: "hd_t", 4: "red",}
...
def print_model_info(model, checkpoint_path):
    ...
    try:
        scale = float(model.fusion_scale.detach().float().cpu())
        print(f"LAB fusion scale：{scale:.4f}")
    except Exception:
        pass
    print("=" * 70)
...
def resolve_checkpoint(checkpoint_dir, checkpoint_name):
    """支持完整路径，或 checkpoint_dir 下的 best/last；兼容 .pt/.pth。"""
    ckpt_path = checkpoint_name

    if (os.sep not in ckpt_path and "/" not in ckpt_path and not os.path.exists(ckpt_path)):
        candidates = []
        if checkpoint_name.lower().endswith((".pt", ".pth")):
            candidates.append(os.path.join(checkpoint_dir, checkpoint_name))
        else:
            candidates.extend([
                os.path.join(checkpoint_dir, checkpoint_name + ".pt"),
                os.path.join(checkpoint_dir, checkpoint_name + ".pth"),
            ])
...
# ║                        模型加载                               ║
# ╚══════════════════════════════════════════════════════════════╝
def load_model(weight_path, device, num_classes):
    """加载 DINOv3 + Lab a* 双输入模型。"""
    hms_cfg = HMSConfig()
    base_cfg = SimpleNamespace(
        backbone_name=hms_cfg.backbone_name,
    ...
    model = HMSLabRedSeg(
        base_cfg,
        red_class=hms_cfg.red_class,
        lab_channels=hms_cfg.lab_channels,
        fusion_init=hms_cfg.lab_fusion_init,
    ).to(device)
    ...
    try:
        print(f"  LAB scale={float(model.fusion_scale.detach().float().cpu()):.4f}")
    except Exception:
        pass
    return model
...
# ║                       推理核心函数                             ║
# ╚══════════════════════════════════════════════════════════════╝
def _predict_probabilities(model, rgb_batch, a_batch):
    """前向推理，返回 [N,C,H,W] 概率。"""
    with torch.no_grad():
        seg, _, _ = model(rgb_batch, a_batch)
...
def _preprocess_batch(images, device):
    ...
    使用训练时相同的 RGB 标准化与 Lab a* 生成方式。
    """
    rgb_list = []
    a_list = []
    for image_rgb in images:
        a = rgb_to_lab_a(image_rgb)
        rgb_list.append(rgb_to_tensor(image_rgb))
        a_list.append(a_to_tensor(a))

    rgb_batch = torch.stack(rgb_list, dim=0).to(device, non_blocking=True)
    a_batch = torch.stack(a_list, dim=0).to(device, non_blocking=True)

    if device.type == "cuda":
        rgb_batch = rgb_batch.half()
        a_batch = a_batch.half()

    return rgb_batch, a_batch
...
def sliding_window_infer(model, image, device, window_size, stride, num_classes):
    ...
    for y1 in ys:
        for x1 in xs:
            ...
            rgb_batch, a_batch = _preprocess_batch(np.expand_dims(patch, axis=0), device)
            probs = _predict_probabilities(model, rgb_batch, a_batch)[0][:, :ph, :pw]

            assert probs.shape[0] == num_classes, (f"模型输出通道数={probs.shape[0]}，配置 num_classes={num_classes}")
    ...
    for class_id in sorted(display_class_ids):
        ...
        for contour in contours:
            ...
            if y >= label_height:
                ...
                label_y1 = min(y + height, max(0, image_height - label_height))
...
def create_output_image(original_image, overlay_image):
    ...
    if output_mode == "side_by_side":
        ...
        raise ValueError(f"不支持的 concat_direction：{concat_direction}，" '请选择 "horizontal" 或 "vertical"')
...
def validate_config():
    ...
    if num_classes != 5:
        raise ValueError("当前红墨水项目定义为 4 个前景类 + 背景，" "因此 num_classes 应为 5。")

    if set(CLASS_COLORS.keys()) != expected_foreground_ids:
        raise ValueError(f"CLASS_COLORS 类别应为 {sorted(expected_foreground_ids)}")

    if set(CLASS_NAMES.keys()) != expected_foreground_ids:
        raise ValueError(f"CLASS_NAMES 类别应为 {sorted(expected_foreground_ids)}")

    invalid_display = display_class_ids - expected_foreground_ids
    if invalid_display:
        raise ValueError(f"display_class_ids 中有无效类别：{sorted(invalid_display)}")
    ...
    if output_mode not in {"overlay", "side_by_side"}:
        raise ValueError('output_mode 只能是 "overlay" 或 "side_by_side"')
...
def main():
    ...
    # 单模型 checkpoint
    checkpoint_path = resolve_checkpoint(INFER["checkpoint_dir"], INFER["checkpoint_name"])
    ...
    print("=" * 70)
    print("加载 DINOv3 + Lab a* 红墨水识别模型...")

    model = load_model(checkpoint_path, device, num_classes)
    ...
    if output_mode == "side_by_side":
        concat_label = ("左右拼接（左原图 / 右叠加图）" if concat_direction == "horizontal" else "上下拼接（上原图 / 下叠加图）")
        print(f"output_mode={output_mode}  " f"concat_direction={concat_direction}  " f"({concat_label})")
    else:
    ...
    image_names = sorted(name for name in os.listdir(test_dir) if name.lower().endswith(VALID_EXTS))
    ...
    for idx, name in enumerate(image_names):
        ...
        # ③ 保存类别索引 mask
        if INFER["save_mask"]:
            save_mask_png(os.path.join(mask_dir, stem + ".png"), mask)

        # ④ 保存原图叠加结果
        save_image(os.path.join(overlay_dir, name), output_image)

        present_ids = [cid for cid in range(1, num_classes) if np.any(mask == cid)]
        present_names = [CLASS_NAMES[cid] for cid in present_ids]
