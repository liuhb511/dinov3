...
INFER = {
    ...
    # "test_image_dir": "F:/liuhaibo/datasets/test/JZW/HQL_JZW_0810/1",   #  莱钢 测试集
    "test_image_dir": "F:/liuhaibo/datasets/test/HMS/test_images",   #  哈汽轮 测试集
    ...
    "stride":            516,                                     # 0=自适应模式, >0=固定步长
    "confidence_threshold": 0.4,                                # 最大类别概率低于该值时设为背景0
    ...
    "output_dir":     "F:/liuhaibo/datasets/test/HMS",                    # 莱钢 输出路径

    "output_subdir":  "test_masks",                   # 输出子路径
...
def _sliding_positions(length, window_size, stride):
    """固定步长模式：从0开始，严格按照 stride 滑动。"""
    if length <= 0:
        raise ValueError(f"length={length} <= 0")

    if window_size <= 0:
        raise ValueError(f"window_size={window_size} <= 0")
...
def sliding_window_infer(model, image, device, window_size, stride):
    ...
    stride > 0 → 按指定步长滑动，边缘不足补灰
    stride = 0 → 自适应均匀分布，首尾贴边
    ...
    if stride > 0:
        ...
        mode_name = f"fixed stride={stride}"
    else:
    ...
    for y1 in ys:
        for x1 in xs:
            ...
            assert probs.shape[0] == num_classes, (
                f"channels: {probs.shape[0]} vs {num_classes}"
            )

            # 只保留真实图像对应的概率区域
            valid_probs = probs[:, :ph, :pw]

            # 重叠区域概率累加
            probability_sum[:, y1:y2, x1:x2] += valid_probs

            # 当前窗口覆盖区域计数 +1
            count_map[y1:y2, x1:x2] += 1.0

    # 正常情况下不会出现未覆盖像素，这里保险处理
    if np.any(count_map == 0):
        raise RuntimeError("存在未被滑动窗口覆盖的像素，请检查 stride 设置。")

    # 重叠区域概率平均
    probability_map = probability_sum / count_map[None, :, :]
...
if __name__ == "__main__":
    device = torch.device(
        INFER["device"] if torch.cuda.is_available() else "cpu"
    )
    ...
    for idx, name in enumerate(image_names):
        ...
        index_mask, confidence = infer_single(
            model,
            path,
            device
        )
