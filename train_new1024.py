...
TRAIN = {
    # ========== 数据集 ==========
    "dataset_root":    "F:/liuhaibo/datasets/BG_HQL_JZW/dataset_crops_1024/ABC",     # 根目录，自动拼接子路径

    # ========== 模型结构（必须与预训练/checkpoint一致）==========
    "backbone_name":   "dinov3_model",
    "freeze_backbone": True,          # 模型构造参数
    "num_classes":     9,             # 4(D类) / 5(ABCD) / 7(ABC) / 5(D+TIN-D) / 9(ABC+TIN-B+TIN-C)
    "image_size":      1024,          
    ...
    "ce_weight":       0.7,
    "dice_weight":     0.3,
    "boundary_weight": 0,             # Boundary Head 默认不启用

    # ========== 保存 / 日志 ==========
    "save_dir":        "./checkpoints/TINBC1024",
    "log_dir":         "./logs/TINBC1024",
