...
# 是否显示实例平均置信度（需 _confidence.png 同级存在）
...
OUTPUT_MODE = "overlay"             #   只输出半透明叠加图。
# OUTPUT_MODE = "side_by_side"        #   左侧为原图，右侧为半透明叠加图。
...
def validate_config() -> None:
    ...
    if len(CLASS_COLORS) != NUM_CLASSES:
        raise ValueError(
            ...
            f"CLASS_COLORS数量={len(CLASS_COLORS)}"
        )
    ...
    if len(CLASS_NAMES) != NUM_CLASSES:
        raise ValueError(
            f"类别数量与类别名称数量不一致："
            f"NUM_CLASSES={NUM_CLASSES}，"
            f"CLASS_NAMES数量={len(CLASS_NAMES)}"
        )
    ...
    if actual_class_ids != expected_class_ids:
        raise ValueError(
            "CLASS_COLORS 的类别编号必须连续，"
            f"应为：{sorted(expected_class_ids)}，"
            f"实际为：{sorted(actual_class_ids)}"
        )

    if actual_name_ids != expected_class_ids:
        raise ValueError(
            "CLASS_NAMES 的类别编号必须连续，"
            f"应为：{sorted(expected_class_ids)}，"
            f"实际为：{sorted(actual_name_ids)}"
        )

    for class_id, color in CLASS_COLORS.items():
        if len(color) != 3:
            raise ValueError(
                f"类别 {class_id} 的颜色必须包含3个通道，"
                f"当前颜色为：{color}"
            )
    ...
    if OUTPUT_MODE not in {
        ...
        raise ValueError(
            'OUTPUT_MODE 只能设置为 '
            '"overlay" 或 "side_by_side"'
        )

    if UNKNOWN_CLASS_POLICY not in {
        "error",
        "ignore",
    }:
        raise ValueError(
            'UNKNOWN_CLASS_POLICY 只能设置为 '
            '"error" 或 "ignore"'
        )
...
def build_mask_index(
    ...
    for mask_path in mask_dir.rglob("*"):
        ...
        if (
            ...
            not in SUPPORTED_MASK_SUFFIXES
        ):
            continue
...
def read_index_mask(
    ...
    if mask is None:
        raise FileNotFoundError(
            f"无法读取Mask：{mask_path}"
        )

    # 如果Mask被保存成三通道，但三个通道完全相同，
    # 则自动提取其中一个通道。
    if mask.ndim == 3:
        if mask.shape[2] == 4:
            mask = mask[:, :, :3]
        ...
        if (
            ...
            raise ValueError(
                f"Mask不是单通道类别索引图：{mask_path.name}"
            )

    if mask.ndim != 2:
        raise ValueError(
            f"Mask维度不正确：{mask_path.name}，"
            f"当前形状为：{mask.shape}"
        )
...
def check_mask_classes(
    ...
    unknown_class_ids = sorted(
        actual_class_ids - valid_class_ids
    )
    ...
    if UNKNOWN_CLASS_POLICY == "error":
        raise ValueError(
            f"Mask {mask_name} 中存在未配置类别："
            f"{unknown_class_ids}；"
            f"允许类别为：{sorted(valid_class_ids)}"
        )

    print(
        f"[提示] Mask {mask_name} 中存在未配置类别："
        f"{unknown_class_ids}，这些类别将被忽略。"
    )
...
def load_confidence_map(confidence_path, target_shape):
    """读取 _confidence.png 归一化到 0~1，不存在返回 None"""
    from pathlib import Path as _Path
    if isinstance(confidence_path, str):
        confidence_path = _Path(confidence_path)
    if not confidence_path.exists():
        return None
    conf = cv2.imread(str(confidence_path), cv2.IMREAD_UNCHANGED)
    if conf is None:
        return None
    if conf.ndim == 3:
        conf = conf[:, :, 0]
    ...
    th, tw = target_shape
    if conf.shape[:2] != (th, tw):
        conf = cv2.resize(conf, (tw, th), interpolation=cv2.INTER_LINEAR)
    return conf
    ...
    for class_id, bgr_color in CLASS_COLORS.items():
        ...
        for contour in contours:
            ...
            if confidence_map is not None:
                ...
                region_conf = confidence_map[contour_mask == 1]
                if len(region_conf) > 0:
                    mean_conf = float(np.mean(region_conf))
                    label_text = f"{class_name}, {mean_conf:.2f}"
...
def create_overlay(
    ...
    if DRAW_CONTOURS:
        overlay = draw_class_contours_and_labels(
            overlay,
            mask,
            confidence_map,
        )
...
def save_image(
    ...
    encode_extension = extension_map.get(
        suffix,
        ".png",
    )
    ...
    for image_path in image_paths:
        ...
        try:
            ...
            if SHOW_CONFIDENCE:
                ...
                conf_path = _P(mask_path.parent) / f"{mask_path.stem}_confidence.png"
                confidence_map = load_confidence_map(conf_path, (height, width))
                if confidence_map is None:
                    print(f"  [提示] 未找到置信度文件: {conf_path.name}")
            ...
            output_image = create_output_image(
                image,
                overlay_image,
            )
