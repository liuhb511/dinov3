...
class Process:
    def __init__(self):
        ...
        # 只统计A、B、C、HH、XW、XQL六个类别
        self.JZW_COUNT = [0, 0, 0, 0, 0, 0]
        self.img_COUNT = [0, 0, 0, 0, 0, 0]
    ...
    def run(self, cropType):
        for imgpath in self.imgs_path:
            ...
            # 只保存包含A/B/C/HH/XW/XQL有效区域的图片
            if np.max(mask) > 0:
                self.enhance(img, mask, imgpath)
                print(imgpath + " ---- Save")
                print("----------------------")
            else:
                print(imgpath + " ---- 不包含A/B/C/HH/XW/XQL，已跳过")
                print("----------------------")
    ...
    def json2voc(self, imgpath):
        ...
        # 记录当前图片中是否出现A、B、C、HH、XW、XQL
        jzw_flag = [0, 0, 0, 0, 0, 0]
...
if __name__ == "__main__":
    ...
    savepath = "F:/liuhaibo/datasets/JZW_v3/crop_1024/ABC_1024/"
