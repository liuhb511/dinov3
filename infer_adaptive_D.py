...
INFER = {
    ...
    # "test_image_dir": "F:/liuhaibo/datasets/test/JZW/DHTG/total",   #  莱钢 测试集
    "test_image_dir": "F:/liuhaibo/datasets/test/JZW/HQL_JZW_0810/3",   #  莱钢 测试集


    # ========== 模型结构（必须与训练时一致）==========
    "backbone_name":   "dinov3_model",
    "freeze_backbone": True,
    "num_classes":     4,            # 4(D类) / 5(ABCD) / 7(ABC) / 5(D+TIN-D) / 9(ABC+TIN-B+TIN-C) / 10(ABC+TIN)

    # ========== 权重加载 ==========
    "checkpoint_dir":  "./checkpoints/D1024",             # 点状（D）类    
    # "checkpoint_dir":  "./checkpoints/ABCTIN",                # 条状（ABC + 氮化钛）
    # "checkpoint_dir":  "./checkpoints/ABC_norotate",          # 条状（ABC，适用于哈汽轮）
    "checkpoint_name": "DDS1024_slim",                              # best_iou / best_dice / last / 完整路径
    ...
    "output_dir":     "F:/liuhaibo/datasets/output/dino/HQL_JZW_0810/3",                     # 哈汽轮 输出路径
    # "output_dir":     "./output/infer_LG",                    # 莱钢 输出路径

    "output_subdir":  "masks_D",                   # 输出子路径
...
if __name__ == "__main__":
    ...
    print(f"num_classes={num_classes}  crop_size={crop_size}  "
          f"mode={mode_label}  close_kernel={infer_close_kernel}")
