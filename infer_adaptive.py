...
INFER = {
    ...
    "test_image_dir": "F:/liuhaibo/datasets/test/JZW/HQL_JZW_0810/3",   #  哈汽轮 测试集


    # ========== 模型结构（必须与训练时一致）==========
    "backbone_name":   "dinov3_model",
    "freeze_backbone": True,
    "num_classes":     7,            # 4(D类) / 5(ABCD) / 7(ABC) / 5(D+TIN-D) / 9(ABC+TIN-B+TIN-C) / 9(ABC+TIN)

    # ========== 权重加载 ==========
    # "checkpoint_dir":  "./checkpoints/D1024",             # 点状（D）类    
    # "checkpoint_dir":  "./checkpoints/ABCTIN1024",                # 条状（ABC + 氮化钛）
    "checkpoint_dir":  "./checkpoints/ABC1024",          # 条状（ABC，适用于哈汽轮）


    "checkpoint_name": "best_iou",                              # best_iou / best_dice / last / 完整路径
    # "checkpoint_name": "ABC1024_slim",                              # best_iou / best_dice / last / 完整路径
